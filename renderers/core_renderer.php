...
class theme_ucl_core_renderer extends theme_essential_core_renderer {
    ...
    protected function render_custom_menu(custom_menu $menu) {
    	...
    	*/
        ...
        if (isloggedin() && !isguestuser() && $hasdisplaymycourses) {
            ...
            if ($mycoursetitle == 'module') {
				$branchtitle = get_string('mymodules', 'theme_ucl');
			} else if ($mycoursetitle == 'unit') {
				$branchtitle = get_string('myunits', 'theme_ucl');
			} else if ($mycoursetitle == 'class') {
				$branchtitle = get_string('myclasses', 'theme_ucl');
			} else {
				$branchtitle = get_string('mycourses', 'theme_ucl');
			}
            ...
            $branch = $menu->add($branchlabel, $branchurl, $branchtitle, $branchsort);
 			if ($courses = enrol_get_my_courses(NULL, 'fullname ASC')) {
                ...
                $noenrolments = get_string('noenrolments', 'theme_ucl');
    	...
    	*/
        ...
        if (isloggedin() && !isguestuser() && $hasdisplaymydashboard) {
            $branchlabel = '<i class="fa fa-dashboard"></i>'.get_string('mydashboard', 'theme_ucl');
            $branchurl   = new moodle_url('/my/index.php');
            $branchtitle = get_string('mydashboard', 'theme_ucl');
            $branchsort  = 10000;
        ...
        if (isloggedin() && !isguestuser()) {
            ...
            if (!empty($alternativethemes)) {
                $branchtitle = get_string('themecolors', 'theme_ucl');
                $branchlabel = '<i class="fa fa-th-large"></i>' . $branchtitle;
                $branchurl   = new moodle_url('/my/index.php');
                $branchsort  = 11000;
                $branch = $menu->add($branchlabel, $branchurl, $branchtitle, $branchsort);
                
                $defaultthemecolorslabel = get_string('defaultcolors', 'theme_ucl');
                $branch->add('<i class="fa fa-square colours-default"></i>' . $defaultthemecolorslabel,
                        new moodle_url($this->page->url, array('uclcolours' => 'default')), $defaultthemecolorslabel);
                foreach ($alternativethemes as $alternativethemenumber) {
                    if (!empty($this->page->theme->settings->{'alternativethemename' . $alternativethemenumber})) {
                        ...
                        $alternativethemeslabel = get_string('alternativecolors', 'theme_ucl', $alternativethemenumber);
                    }
                    $branch->add('<i class="fa fa-square colours-alternative' .  $alternativethemenumber . '"></i>' . $alternativethemeslabel,
                            new moodle_url($this->page->url, array('uclcolours' => 'alternative' . $alternativethemenumber)), $alternativethemeslabel);
                }
            }
        }
    ...
    */
    ...
    public function uclblocks($region, $classes = array(), $tag = 'aside') {
...
class theme_ucl_format_topics_renderer extends theme_essential_format_topics_renderer {
    ...
    protected function get_nav_links($course, $sections, $sectionno) {
        ...
        while ($back > 0 and empty($links['previous'])) {
            if ($canviewhidden || $sections[$back]->uservisible) {
                ...
                $previouslink .= get_string('previoussection', 'theme_ucl');
        ...
        while ($forward <= $course->numsections and empty($links['next'])) {
            if ($canviewhidden || $sections[$forward]->uservisible) {
                ...
                $nextlink .= get_string('nextsection', 'theme_ucl');
